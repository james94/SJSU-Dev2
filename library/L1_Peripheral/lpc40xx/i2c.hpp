...
class I2c final : public sjsu::I2c
{
 ...
 private:
  void ConfigureClockRate()
  {
    ...
    const float kClockFrequency = kPeripheralFrequency / settings.frequency;
    const float kScll           = kClockFrequency * settings.duty_cycle;
    const float kSclh           = kClockFrequency * (1 - settings.duty_cycle);
