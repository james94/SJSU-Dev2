...
class Mma8452q : public Accelerometer
{
 public:
  ...
  struct Map
  {
    ...
    static constexpr auto kStatus = DeviceMemoryMap::Register(
        { .spec = kSpec, .address = 0x00, .width = 1 });

    /// Register address of the the first byte of the X axis
    static constexpr auto kXYZStartAddress = DeviceMemoryMap::Register(
        { .spec = kSpec, .address = 0x01, .width = 6 });

    /// Device ID register address
    static constexpr auto kWhoAmI = DeviceMemoryMap::Register(
        { .spec = kSpec, .address = 0x0D, .width = 1 });

    /// Device configuration starting address
    static constexpr auto kDataConfig = DeviceMemoryMap::Register(
        { .spec = kSpec, .address = 0x0E, .width = 1 });

    /// Control register 1 holds the enable bit
    static constexpr auto kControlReg1 = DeviceMemoryMap::Register(
        { .spec = kSpec, .address = 0x2A, .width = 1 });
  ...
  Returns<Acceleration_t> Read() override
  {
    ...
    auto memory                     = device_.CreateMemoryMapIO();
    std::array<int16_t, 3> xyz_data = memory[Map::kXYZStartAddress];

    if (device_.GetError())
    {
      return Error(device_.GetError());
    }
  ...
  Returns<void> SetFullScaleRange()
  {
    ...
    memory[Map::kDataConfig] = static_cast<uint8_t>(gravity_scale >> 2);
  ...
  Returns<void> ActiveMode(bool is_active = true)
  {
    ...
    memory[Map::kControlReg1] = is_active;
  ...
  Returns<void> IsValidDevice()
  {
    ...
    uint8_t device_id = memory[Map::kWhoAmI];
    SJ2_CHECK_DEVICE_ERROR(device_);
